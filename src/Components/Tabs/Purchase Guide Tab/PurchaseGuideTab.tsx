...
const PurchaseGuideTab: React.FC<PurchaseGuideTabProps> = ({
  ...
  const filteredSeries = useFavourites
    ...
    ? seriesList.filter((series) =>
        fullSeasonSchedule[series].eligibleCars.some(
          (car) => carData[car].owned
        )
      )
    : [...seriesList];
  ...
  const unownedTracks = trackList.filter(
    (track) => trackData[track].owned === false
  );

  // Order tracks by usage
  unownedTracks.sort(
    (a, b) => trackUsageData[b].usage - trackUsageData[a].usage
  );
  unownedTracks.forEach((track) => {
    if (trackUsageData[track].usage !== 0) {
      console.log(
        `Track: ${trackData[track].name}. Usage: ${trackUsageData[track].usage}.`
      );
  ...
  return (
    <div className="purchase-guide-wrapper">
      ...
      <p>
        Use this page to see which of your unowned tracks are used the most in
        the current season. This guide does not use data for previous weeks to
        aid in its suggestions.
      </p>
